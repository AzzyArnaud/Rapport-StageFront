...
export default class Validation {
    ...
    if (this.data[params] !== value) {
      this.setError(
        ...
        this.customMessages?.[key]?.match ||
          `La valeur ne correspond pas à la valeur ${params}`
      );
    }
  }
  ...
  email(key, value) {
    ...
    if (!validEmail) {
      this.setError(
        key,
        this.customMessages?.[key]?.email || "Adresse Email incorrecte"
      );
    }
  }
    ...
    if (!isnum) {
      this.setError(
        ...
        this.customMessages?.[key]?.number ||
          `Ce champ doit être un numéro valide`
      );
    }
  }
  ...
  string(key, value) {
    ...
    if (!isString) {
      this.setError(
        key,
        this.customMessages?.[key]?.string ||
          `Ce champ doit être une chaîne valide`
      );
    }
  }
    ...
    if (!isString) {
      this.setError(
        ...
        this.customMessages?.[key]?.alpha ||
          `Ce champ doit contenir uniquement des caractères alphanumériques`
      );
    }
  }
  ...
  image(key, value, params) {
    ...
    if (value) {
      if (!IMAGES_MIMES.includes(value.type)) {
        this.setError(
          key,
          this.customMessages?.[key]?.image ||
            "Veuillez choisir une image valide"
        );
      } else if (params < value.size) {
        const megaByte = (params - 1000000) / 1000000;
        this.setError(
          key,
          this.customMessages?.[key]?.size ||
            `Votre image est trop grande (max: ${megaByte} MB)`
        );
      }
    }
  }
    ...
    if (params < value.size) {
      ...
      this.setError(
        ...
        this.customMessages?.[key]?.fileSize ||
          `File too large (max: ${megaByte} MB)`
      );
    }
  }

  optional(key, value) {
    // La méthode optional peut être définie comme un champ optionnel
    if (!value) return;

    // Vous pouvez ajouter des validations supplémentaires ici si nécessaire
    // Exemple : vérifier que le champ est une chaîne de caractères
    if (typeof value !== "string") {
      this.setError(
        key,
        this.customMessages?.[key]?.optional ||
    ...
    for (let key in this.validation) {
      ...
      properties.forEach((property) => {
        if (typeof this[property] === "function") {
          ...
          console.error(`Méthode de validation ${property} non définie`);
        }
      });
    }
        ...
        if (this.customValidations[key].length > 0) {
          this.customValidations[key].forEach((customValidation) => {
            if (!customValidation.isValid(this.data)) {
              this.setError(
                key,
                this.customMessages[key]?.[customValidation.validationName] ||
                  `Message d'erreur non spécifié`
              );
            }
          });
        }
